...
# rpc-amoy.polygon.technology đã bị Polygon deprecate (17/7) — dùng danh sách dự phòng,
# tự động thử endpoint kế tiếp nếu cái đầu không kết nối được.
AMOY_RPC_URLS = [
    'https://polygon-amoy-bor-rpc.publicnode.com',
    'https://polygon-amoy.drpc.org',
    'https://rpc-amoy.polygon.technology',  # giữ lại cuối cùng phòng khi được khôi phục
]

def _connect_web3():
    for url in AMOY_RPC_URLS:
        try:
            candidate = Web3(Web3.HTTPProvider(url, request_kwargs={'timeout': 6}))
            if candidate.is_connected():
                print(f"[web3] Đã kết nối RPC: {url}")
                return candidate
        except Exception as e:
            print(f"[web3] RPC lỗi ({url}): {e}")
    print("[web3] CẢNH BÁO: không kết nối được RPC nào, dùng endpoint đầu tiên (sẽ lỗi khi ghi chain).")
    return Web3(Web3.HTTPProvider(AMOY_RPC_URLS[0]))
...
def init_db():
    ...
    for col, coltype in [('owner_id', 'INTEGER'), ('is_public', 'INTEGER DEFAULT 0'), ('webhook_token', 'TEXT')]:
        try:
...
def save_fund(fund_id, name, bank, stk, owner_id, is_public, webhook_token=None):
    conn = sqlite3.connect(DB_PATH)
    conn.execute('INSERT INTO funds (fund_id, name, bank, stk, created_at, owner_id, is_public, webhook_token) VALUES (?,?,?,?,?,?,?,?)',
        (fund_id, name, bank, stk, int(time.time()), owner_id, 1 if is_public else 0, webhook_token))
    conn.commit()
    conn.close()

def set_fund_webhook_token(fund_id, token):
    conn = sqlite3.connect(DB_PATH)
    conn.execute('UPDATE funds SET webhook_token=? WHERE fund_id=?', (token, fund_id))
    conn.commit()
    conn.close()
...
FUND_COLS = "fund_id, name, bank, stk, created_at, owner_id, is_public, webhook_token"
...
def add_expense(fund_id, amount, desc, to_fund, user_id):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.execute('INSERT INTO expenses (fund_id, amount, desc, to_fund, created_at, created_by) VALUES (?,?,?,?,?,?)',
...
def send_record_tx(fund_id, bank_tx_id, amount, content, is_income=True):
    """Ghi 1 giao dịch (thu HOẶC chi) lên smart contract.
    is_income=True  -> tiền vào (SePay webhook phát hiện)
    is_income=False -> chi tiêu (chủ quỹ tự khai qua form 'Ghi chi tiêu')
    """
    try:
        nonce = w3.eth.get_transaction_count(account_address)
        tx = contract.functions.recordTransaction(fund_id, bank_tx_id, amount, content, is_income).build_transaction({
        ...
        signed = w3.eth.account.sign_transaction(tx, private_key=PRIVATE_KEY)
        tx_hash = w3.eth.send_raw_transaction(signed.raw_transaction)
        kind = "thu" if is_income else "chi"
        print(f"✅ recordTx [{fund_id}] ({kind}) {amount}đ tx: {tx_hash.hex()}")
    except Exception as e:
        kind = "thu" if is_income else "chi"
        print(f"❌ recordTx ({kind}) error: {e}")
...
def index():
    ...
    if request.method == 'POST':
        ...
        webhook_token = ''.join(random.choices(string.ascii_lowercase + string.digits, k=24))
        save_fund(fund_id, name, bank, stk, owner_id, is_public, webhook_token)
...
def _fund_row(fund, badge=None):
    icons = {'VCB':'🟢','BIDV':'🔵','ICB':'💙','MB':'⭐','ACB':'🔷','VPB':'🌿','TPB':'🟣',
             'STB':'🟤','MSB':'🔴','OCB':'🟡','ABB':'🟠','KLB':'🔶'}
...
def view_fund(fund_id):
    ...
    if not fund.get('webhook_token'):
        _new_token = ''.join(random.choices(string.ascii_lowercase + string.digits, k=24))
        set_fund_webhook_token(fund_id, _new_token)
        fund['webhook_token'] = _new_token
  ...
  <div class="member-chips">{chips}</div>
    ...
    webhook_block = ''
    if user and fund['owner_id'] == user['id']:
        webhook_url = request.url_root.rstrip('/') + f"/webhook/{fund_id}/{fund['webhook_token']}"
        webhook_block = f"""<div class="card reveal">
  <div class="sh"><div class="sh-title">🔌 Tự động ghi nhận chuyển khoản (SePay)</div></div>
  <p style="font-size:12.5px;color:var(--text2);line-height:1.75;margin-bottom:14px">
    Quỹ này chưa tự động phát hiện giao dịch. Để mọi khoản chuyển vào đúng tài khoản
    <strong style="color:var(--text)">{fund['bank']} · {fund['stk']}</strong> của bạn tự động
    ghi lên blockchain, làm theo 3 bước sau — Fundy không giữ tiền của bạn, tiền vẫn về thẳng tài khoản của bạn.
  </p>
  <div class="steps">
    <div class="step">
      <div class="step-num">1</div>
      <div class="step-body">
        <div class="step-title">Đăng ký SePay (miễn phí)</div>
        <div class="step-desc">Vào <a href="https://my.sepay.vn" target="_blank" rel="noopener">my.sepay.vn</a>, tạo tài khoản nếu bạn chưa có.</div>
      </div>
    </div>
    <div class="step">
      <div class="step-num">2</div>
      <div class="step-body">
        <div class="step-title">Liên kết đúng tài khoản {fund['bank']} · {fund['stk']}</div>
        <div class="step-desc">Chọn "SMS Báo số dư" (hoặc API ngân hàng nếu có), xác nhận qua OTP điện thoại — không cần nhập mật khẩu ngân hàng.</div>
      </div>
    </div>
    <div class="step">
      <div class="step-num">3</div>
      <div class="step-body">
        <div class="step-title">Dán link Webhook riêng của quỹ này vào SePay</div>
        <div class="step-desc">Vào mục Webhooks trên SePay → Thêm mới → chọn đúng tài khoản ngân hàng vừa liên kết → dán link dưới đây:</div>
        <div class="share-box" style="margin-top:8px">
          <span class="share-url">{webhook_url}</span>
          <button class="copy-btn" onclick="copyLink('{webhook_url}')">{T('copy_btn')}</button>
        </div>
      </div>
    </div>
  </div>
  <div class="notice" style="margin-top:14px">
    <span>🔒</span> Link này là bí mật — ai có link này có thể ghi giao dịch giả vào quỹ, đừng chia sẻ công khai.
  </div>
</div>"""
...
<div class="expense-section reveal">
  ...
  {'<div class="expense-list">' + ''.join(
    ...
    webhook_open = 'open' if (user and fund.get('owner_id') == (user['id'] if user else None) and count == 0) else ''
...
<details class="collapsible-section reveal" style="margin-top:16px;background:var(--surface,#14141c);border:1px solid var(--border,#2a2a35);border-radius:14px;padding:0 18px">
  <summary style="cursor:pointer;padding:16px 0;font-size:14px;font-weight:600;color:var(--text2,#c9c9d6);list-style:none">💸 Quản lý chi tiêu quỹ</summary>
  <div style="padding-bottom:16px">{expense_block}</div>
</details>

<details class="collapsible-section reveal" {webhook_open} style="margin-top:12px;background:var(--surface,#14141c);border:1px solid var(--border,#2a2a35);border-radius:14px;padding:0 18px">
  <summary style="cursor:pointer;padding:16px 0;font-size:14px;font-weight:600;color:var(--text2,#c9c9d6);list-style:none">🔌 Tự động ghi nhận chuyển khoản (SePay){' — chưa thiết lập' if count == 0 and webhook_block else ''}</summary>
  <div style="padding-bottom:16px">{webhook_block}</div>
</details>

<!-- SANKEY DIAGRAM -->
<div class="sankey-wrap reveal" id="sankeyWrap" style="display:{'none' if not sankey_links else 'block'};margin-top:16px">
...
def add_expense_route(fund_id):
    ...
    try:
        ...
        if amount <= 0 or not desc:
            ...
            return redirect(url_for('view_fund', fund_id=fund_id))
        expense_id  = add_expense(fund_id, amount, desc, to_fund, user['id'])
        # Chi tiêu cũng phải minh bạch như tiền vào — ghi lên chain, chạy nền
        # để không chặn phản hồi cho người dùng. Vì không có mã GD ngân hàng
        # (đây là khoản chi tự khai bởi chủ quỹ, không phải tiền vào), dùng
        # mã tham chiếu riêng "EXP-<id>" để phân biệt trên Polygonscan.
        expense_ref = f"EXP-{expense_id}"
        threading.Thread(target=send_record_tx, args=(fund_id, expense_ref, amount, desc, False), daemon=True).start()
...
@app.route('/webhook/<fund_id>/<token>', methods=['POST'])
def webhook_fund(fund_id, token):
    fund = get_fund(fund_id)
    # The secret token in the URL IS the authentication — no fund exists,
    # or the token doesn't match this fund's token: reject.
    if not fund or not fund.get('webhook_token') or fund['webhook_token'] != token:
    ...
    print(f"📨 Webhook [{fund_id}]: {amount}đ | {content}")

    threading.Thread(target=send_record_tx, args=(fund['fund_id'], bank_tx_id, amount, content, True), daemon=True).start()

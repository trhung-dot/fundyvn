...
TRANSLATIONS = {
    "vi": {
        ...
        "hero_cta":       "Bắt đầu tạo quỹ →",
        "hero_cta_sub":   "Miễn phí · Không cần thẻ ngân hàng để dùng thử",
        ...
        "how_title":      "Cách Fundy hoạt động",
        "how_sub":        "Ba bước đơn giản, minh bạch từ đầu đến cuối",
        "how1_title":     "Tạo quỹ trong 30 giây",
        "how1_desc":      "Đặt tên quỹ, chọn ngân hàng nhận tiền — không cần thủ tục rườm rà.",
        "how2_title":     "Chia sẻ mã QR",
        "how2_desc":      "Mọi người quét mã và chuyển khoản trực tiếp qua app ngân hàng quen thuộc.",
        "how3_title":     "Theo dõi minh bạch",
        "how3_desc":      "Từng giao dịch tự động ghi lên blockchain — ai cũng kiểm chứng được, không thể sửa.",
        "form_title":     "Tạo quỹ mới",
        "f_name":         "Tên quỹ",
        "f_name_ph":      "Tên quỹ của bạn",
        "f_bank":         "Ngân hàng",
        "f_bank_ph":      "— Chọn —",
        "f_stk":          "Số tài khoản",
        ...
        "reg_username_ph":"Nhập tên đăng nhập",
        "reg_password":   "Mật khẩu",
        "reg_password_ph":"Nhập mật khẩu",
    ...
    "en": {
        ...
        "hero_cta":       "Start a fund →",
        "hero_cta_sub":   "Free · No bank card needed to try it out",
        ...
        "how_title":      "How Fundy works",
        "how_sub":        "Three simple steps, transparent from start to finish",
        "how1_title":     "Create a fund in 30 seconds",
        "how1_desc":      "Name your fund and pick a receiving bank — no paperwork required.",
        "how2_title":     "Share the QR code",
        "how2_desc":      "Everyone scans and transfers directly from the banking app they already use.",
        "how3_title":     "Track it transparently",
        "how3_desc":      "Every transaction is written to the blockchain automatically — anyone can verify it, no one can edit it.",
        "form_title":     "Create a new fund",
        "f_name":         "Fund name",
        "f_name_ph":      "Your fund's name",
        "f_bank":         "Bank",
        "f_bank_ph":      "— Select —",
        ...
        "reg_username_ph":"Enter a username",
        "reg_password":   "Password",
        "reg_password_ph":"Enter a password",
...
body{
  ...
  animation:bodyFadeIn .45s ease both;
}
...
.anim-fade-up{
  opacity:0;
  animation:fadeUp .55s cubic-bezier(.2,.7,.3,1) forwards;
  animation-delay:.05s;
}
.anim-slide-right{
  opacity:0;
  animation:slideRight .45s cubic-bezier(.2,.7,.3,1) forwards;
}
.anim-count{opacity:0;animation:fadeUp .5s ease forwards}
.anim-count.d1{animation-delay:.05s}
.anim-count.d2{animation-delay:.15s}
.anim-count.d3{animation-delay:.25s}

/* Scroll reveal (paired with JS IntersectionObserver toggling .revealed) */
.reveal{
  opacity:0;
...
/* Buttons / cards get a gentle lift on hover */
.card, .feat-card, .fund-row, .stat, .step, .how-card{
...
.nav{
  ...
  gap:12px 16px;row-gap:10px;flex-wrap:wrap;
}
.nav-left{display:flex;align-items:center;gap:28px;flex-wrap:wrap;row-gap:8px}
.logo{
...
.nav-right{display:flex;align-items:center;gap:10px;flex-wrap:wrap;row-gap:8px;justify-content:flex-end}
.chain-pill{
...
.nav-btn:hover{border-color:var(--border2);color:var(--text);background:rgba(255,255,255,.07);text-decoration:none}
...
.dropdown-panel{
  ...
  z-index:200;
  animation:dropdownFadeIn .16s ease both;
}
...
.hero-cta-wrap{margin-top:32px;display:flex;flex-direction:column;align-items:center;gap:10px}
.hero-cta{
  display:inline-flex;align-items:center;gap:8px;
  padding:15px 32px;
  background:var(--green);color:#000;
  border-radius:12px;font-weight:700;font-size:15px;
  box-shadow:0 0 28px rgba(34,212,160,.25);
  transition:transform .15s,box-shadow .15s,background .15s;
}
.hero-cta:hover{transform:translateY(-2px) scale(1.02);box-shadow:0 8px 32px rgba(34,212,160,.35);background:var(--green2);color:#000;text-decoration:none}
.hero-cta-sub{font-size:12px;color:var(--text3)}
...
.how-desc{font-size:12.5px;color:var(--text3);line-height:1.7}
...
.fund-row:hover{border-color:var(--border2);background:rgba(255,255,255,.03);transform:translateX(2px)}
.fund-avatar{
...
.modal{
  ...
  animation:popIn .22s cubic-bezier(.2,.7,.3,1) both;
}
.modal-head{
  display:flex;align-items:center;justify-content:space-between;
  padding:20px 24px 0;
...
def nav_html(active=''):
    ...
    if user:
        ...
        right = f"""
        <a class="nav-btn primary" href="/tao-quy">{T('f_submit')}</a>
...
def page(content, active='', narrow=False, wide=False):
    width_cls = ' narrow' if narrow else (' wide' if wide else '')
...
<body>
  ...
  <script>
    ...
    (function(){{
      ...
      document.querySelectorAll('.anim-slide-right').forEach((el,i)=>{{
...
@app.route('/')
def index():
    """Landing / intro page — explains what Fundy is, then points to the
    actual fund-creation service via a CTA button instead of showing the
    service form directly on the homepage."""
    c = f"""
<div class="hero anim-fade-up">
  ...
  <div class="hero-cta-wrap">
    <a class="hero-cta" href="/tao-quy">{T('hero_cta')}</a>
    <span class="hero-cta-sub">{T('hero_cta_sub')}</span>
  </div>
</div>
...
<div class="reveal">
  <div class="sh" style="margin-bottom:4px"><div class="sh-title" style="font-size:16px">{T('how_title')}</div></div>
  <p style="font-size:12.5px;color:var(--text3);margin-bottom:4px">{T('how_sub')}</p>
  <div class="how-grid">
    <div class="how-card">
      <div class="how-num">1</div>
      <div class="how-title">{T('how1_title')}</div>
      <div class="how-desc">{T('how1_desc')}</div>
    </div>
    <div class="how-card">
      <div class="how-num">2</div>
      <div class="how-title">{T('how2_title')}</div>
      <div class="how-desc">{T('how2_desc')}</div>
    </div>
    <div class="how-card">
      <div class="how-num">3</div>
      <div class="how-title">{T('how3_title')}</div>
      <div class="how-desc">{T('how3_desc')}</div>
    </div>
  </div>
</div>
...
<div class="card reveal" style="text-align:center">
  <div class="sh-title" style="margin-bottom:10px">{T('form_title')}</div>
  <p style="font-size:13px;color:var(--text2);margin-bottom:18px">{T('hero_cta_sub')}</p>
  <a class="btn" href="/tao-quy" style="display:inline-block;width:auto;padding:13px 32px">{T('hero_cta')}</a>
</div>"""
    return page(c, active='home')


@app.route('/tao-quy', methods=['GET', 'POST'])
def create_fund_page():
    """The actual fund-creation service — separated from the homepage so
    the homepage can act as a simple intro/landing page first."""
    lang = get_lang()
    if request.method == 'POST':
        ...
        if not name or not bank or not stk:
            ...
            return redirect(url_for('create_fund_page'))
    ...
    c = f"""
<div class="hero anim-fade-up" style="padding-top:24px;padding-bottom:24px">
  <div class="hero-eyebrow">✦ {T('hero_tag')}</div>
  <h1 style="font-size:clamp(24px,4vw,36px)">{T('form_title')}</h1>
</div>
...
def register_page():
    lang = get_lang()
    if current_user(): return redirect(url_for('create_fund_page'))
    if request.method == 'POST':
    ...
    c = f"""
<div class="auth-hero anim-fade-up">
  <div class="auth-logo">Fund<span>y</span></div>
  <div class="auth-sub">{T('reg_sub')}</div>
</div>
<div class="card anim-fade-up">
  <form method="POST">
    <div class="fg"><label>{T('reg_username')}</label>
      <input type="text" name="username" placeholder="{T('reg_username_ph')}" required autofocus></div>
    <div class="fg"><label>{T('reg_password')}</label>
      <input type="password" name="password" placeholder="{T('reg_password_ph')}" required></div>
    <div class="fg"><label>{T('reg_confirm')}</label>
      <input type="password" name="confirm" placeholder="{T('reg_confirm_ph')}" required></div>
    <button class="btn" type="submit">{T('reg_submit')}</button>
  </form>
  <p class="fg-hint" style="text-align:center;margin-top:14px">
    {T('reg_have_acct')} <a href="/dang-nhap">{T('nav_login')}</a></p>
</div>"""
...
<div class="card anim-fade-up">
  ...
  <p class="fg-hint" style="text-align:center;margin-top:14px">
    {T('login_no_acct')} <a href="/dang-ky">{T('nav_register')}</a></p>
</div>"""
    return page(c, narrow=True)
...
def _fund_row(fund, badge=None):
    ...
    return f"""<div class="fund-row anim-slide-right">
...
def community_funds():
    ...
    rows = ''.join(_fund_row(f) for f in funds) if funds else \
        f'<div class="empty-state"><div class="empty-icon">🌐</div>{T("community_empty")}</div>'
    c = f"""
<div class="hero anim-fade-up" style="padding-top:24px">
  <div class="hero-eyebrow">✦ {T('community_tag')}</div>
...
<div class="card anim-fade-up">
  <div class="sh">
    ...
    <span class="sh-meta">{len(funds)} {T('funds')}</span>
  </div>
...
def my_funds():
    ...
    o_rows = ''.join(_fund_row(f, badge='public' if f['is_public'] else 'private') for f in owned) if owned \
        else f'<div class="empty-state"><div class="empty-icon">📭</div>{T("mine_empty")}</div>'

    s_block = ''
    if shared:
        s_rows = ''.join(_fund_row(f, badge='public' if f['is_public'] else 'private') for f in shared)
        s_block = f"""<div class="card anim-fade-up">
  <div class="sh"><div class="sh-title">{T('mine_shared')}</div>
    <span class="sh-meta">{len(shared)} {T('funds')}</span></div>
